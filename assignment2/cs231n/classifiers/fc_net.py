...
def affine_relu_forward_bn(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that perorms an affine transform followed by a BN + ReLU
...
def affine_relu_backward_bn(dout, cache):
    """
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grads.update({'W1':dW1,
                    ...
                    'b2':db2})
        # C = W2.shape[1]
        # y_oh = np.arange(C) == y.reshape(-1, 1)
        # loss = np.sum(- y_oh * np.log(scores))
        # loss /= N
        # loss += 0.5 * self.reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2)

        # dscores = scores - y_oh         # dscores.shape = [N, C]
        # da1 = np.dot()
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if not self.use_batchnorm:
                layers[i], caches[i] = affine_relu_forward(
                    layers[i - 1], 
                    self.params['W' + str(i)],
                    self.params['b' + str(i)]
                    )
            else:
                layers[i], caches[i] = affine_relu_forward_bn(
                    layers[i - 1], 
                    self.params['W' + str(i)],
                    self.params['b' + str(i)],
            ...
            if self.use_dropout:
                layers[i],  caches_dropout[i] = dropout_forward(
                    layers[i],
                    self.dropout_param
                )

        W_out = 'W' + str(self.num_layers)
        b_out = 'b' + str(self.num_layers)
        ...
        dx = {}
        dx[self.num_layers], grads[W_out], grads[b_out]\
        = affine_backward(dscores, cache_scores)
        grads[W_out] += self.reg * self.params[W_out]
        for i in reversed(range(1, self.num_layers)):
            if self.use_dropout:
                dx[i + 1] = dropout_backward(dx[i + 1], caches_dropout[i])
            if not self.use_batchnorm:
                dx[i], grads['W' + str(i)], grads['b' + str(i)]\
                = affine_relu_backward(dx[i + 1], caches[i])
                grads['W' + str(i)] += self.reg * self.params['W' + str(i)]
            else:
                dx[i], grads['W' + str(i)], grads['b' + str(i)], \
                 grads['gamma' + str(i)], grads['beta' + str(i)] \
                = affine_relu_backward_bn(dx[i + 1], caches[i])
                grads['W' + str(i)] += self.reg * self.params['W' + str(i)]
